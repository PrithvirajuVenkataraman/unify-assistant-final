...
export const CRAWLER_INDEX_NAME = 'jarvis_web_pages';
...
export function createMeiliDocumentId(url = '') {
...
export function buildCrawlerDocument({ url, html, fetchedAt = new Date().toISOString(), trusted = false } = {}) {
    ...
    return {
        id: createMeiliDocumentId(canonicalUrl || normalizedUrl),
        url: normalizedUrl,
        canonicalUrl,
...
export async function crawlSeeds(options = {}) {
    ...
    if (options.index !== false && documents.length) {
        await upsertMeiliDocuments(documents, options);
    }
    return { documents, crawledCount: visited.size };
}

export async function configureMeiliIndex(options = {}) {
    const index = options.indexName || process.env.MEILI_INDEX || CRAWLER_INDEX_NAME;
    await meiliFetch(`/indexes/${encodeURIComponent(index)}/settings/searchable-attributes`, {
        method: 'PUT',
        body: JSON.stringify(['title', 'description', 'text', 'domain'])
    }, options);
    await meiliFetch(`/indexes/${encodeURIComponent(index)}/settings/filterable-attributes`, {
        method: 'PUT',
        body: JSON.stringify(['domain', 'publishedAt', 'fetchedAt', 'trusted'])
    }, options);
    await meiliFetch(`/indexes/${encodeURIComponent(index)}/settings/sortable-attributes`, {
        method: 'PUT',
        body: JSON.stringify(['publishedAt', 'fetchedAt'])
    }, options);
}

export async function upsertMeiliDocuments(documents = [], options = {}) {
    if (!documents.length) return null;
    const index = options.indexName || process.env.MEILI_INDEX || CRAWLER_INDEX_NAME;
    return meiliFetch(`/indexes/${encodeURIComponent(index)}/documents?primaryKey=id`, {
        method: 'POST',
...
async function meiliFetch(path, init = {}, options = {}) {
    const host = String(options.meiliHost || process.env.MEILI_HOST || '').replace(/\/+$/, '');
    const key = String(options.meiliAdminKey || process.env.MEILI_ADMIN_KEY || process.env.MEILI_SEARCH_KEY || '').trim();
    if (!host || !key) throw new Error('MEILI_HOST and MEILI_ADMIN_KEY are required for crawler indexing.');
